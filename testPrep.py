...
# 读取数据
ECG = sio.loadmat('Data/Test/ECG_TestData.mat')
data = ECG['Data'][0][0][0] # 第一个字段：表示ECG采样数据
fs = ECG['Data'][0][0][1] # 第三个字段：采样率
fs = fs[0][0]
...
# 展示心电图
def showECG(ecgData, deNoise=True, maxLen=None):
    '''
    ecgData: 某个患者的ECG数据，包含4种情绪数据
    deNoise: 表示是否需要去噪
    maxLen: 设定的样本截取长度，若为None则表示输出所有长度
    '''
    for i in range(4):  # 对于这个人的某一种情绪（共4种）

        sample_rate = fs  # 采样频率
        x = denoise(ecgData[i]) if deNoise else ecgData[i]  # 该情绪对应数据
        x = x[:maxLen] if maxLen else x  # 截取数据的长度
        sample_count = len(x)  # 采样数
        t = np.linspace(0, sample_count / sample_rate, sample_count)

        # 作图
        plt.figure(figsize=(20, 10))
        ax0 = plt.subplot(311)  # 每个图片视为”3行1列“的子图，从第1个位置开始放图
...
for m in range(len(data)):#被试个数
    a = ECG2csv(data[m][0][0], 'temp/myTemp.csv')
    feature = []
    qrs_detector = QRSDetectorOffline(ecg_data_path='temp/myTemp.csv',
...
output.to_csv('test_data.csv',encoding='utf-8',float_format='%.9f',index=None)
# 删除临时文件
shutil.rmtree('logs')
shutil.rmtree('plots')
shutil.rmtree('temp')
